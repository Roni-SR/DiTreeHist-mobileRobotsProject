...
DEFAULT_CONFIG = {
    ...
    # checkpoints
    "ckpt_dir": "checkpoints_newLossMetaRNNNormImp",
...
def train(cfg: Dict):
    ...
    for epoch in range(cfg["epochs"]):
        ...
        for batch in loader:
            ...
            local = ensure_nchw_single_channel(batch["local_map"].to(device))  # (B,1,H,W) in [-1,1]
            ...
            eps_hat = model(
                noisy_actions=x_t,
                history=act_hist,  # (B,H,2)
                goal=goal,  # in [-1,1]
                local_map=local,  # in [-1,1]
                t=t
            )
            ...
            pred_actions_norm = x0_hat.reshape(B, P, D).contiguous()
            pred_actions_seq = denorm_actions_from_pm1(pred_actions_norm, cfg["max_speed"], cfg["max_steer"])  # (B, P, 2) physical
    ...
    torch.save({"model": model.state_dict(), "cfg": cfg, "ema": ema.state_dict()}, ckpt_path)
...
class DiffusionTreePlanner:
    ...
    @torch.no_grad()
    def sample_actions_k(self, node, goal, act_hist=None, K=4):
        best = None
        best_score = -1e9
        best_actions = None
        for _ in range(K):
            actions = self.sample_actions(node, goal, act_hist)
            gx, gy = goal
            x0, y0 = node.state.x, node.state.y
            x1, y1 = x0, y0
            score = 0.0
            collided = False
            cur = node.state
            for a in actions:
                nxt, col = self.env.step(cur, (float(a[0]), float(a[1])))
                if col:
                    collided = True;
                    break
                cur = nxt
            if not collided:
                ...
                score = (d0 - d1) - 0.01 * len(actions)
                if score > best_score:
                    best_score = score
                    best_actions = actions
        return best_actions if best_actions is not None else self.sample_actions(node, goal, act_hist)
...
def visualize_map_and_path(grid: np.ndarray,
    ...
    free = light gray, obstacle = black.
    """
    # 0=free, 1=obstacle  ->  free:#d3d3d3 , obstacle:#000000
    cmap = ListedColormap(["#d3d3d3", "#000000"])
    fig, ax = plt.subplots(figsize=(8, 8))

    # show grid in grayscale: 0=free (white), 1=obstacle (black)
    ax.imshow(grid, cmap="gray", vmin=0, vmax=1)
    im = ax.imshow(grid.astype(np.uint8), cmap=cmap, vmin=0, vmax=1, interpolation="nearest")
    ...
    if start_xy is not None:
        start_scatter = ax.scatter([start_xy[0]], [start_xy[1]],
                                   s=60, marker="*", color="#22c55e", edgecolor="black", linewidths=0.6,
                                   label="Start")
    if goal_xy is not None:
        goal_scatter = ax.scatter([goal_xy[0]], [goal_xy[1]],
                                  s=60, marker="*", color="#ef4444", edgecolor="black", linewidths=0.6,
                                  label="Goal")
    ...
    if handles:
        legl = fig.legend(handles, labels, loc="upper left", bbox_to_anchor=(1.02, 1.0), borderaxespad=0.0, frameon=True, framealpha=0.95)
        legr = ax.legend(handles, labels, loc="upper right", frameon=True, framealpha=0.9)
        handles = getattr(legr, "legendHandles", None) or getattr(legr, "legend_handles", None)
        legend_handles = getattr(legl, "legendHandles", None) or getattr(legl, "legend_handles", None)
